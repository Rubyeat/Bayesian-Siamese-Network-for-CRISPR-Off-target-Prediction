...
class CRISPRNet(nn.Module):
    def __init__(self, seq_len=23, input_dim=7, lstm_hidden=32, dropout_prob=0.3):
        super().__init__()
        self.conv1 = nn.Conv1d(input_dim, 32, 3, padding=1)
        self.conv2 = nn.Conv1d(32, 32, 5, padding=2)
        self.dropout1 = nn.Dropout(dropout_prob)
        self.bilstm = nn.LSTM(32, lstm_hidden, batch_first=True, bidirectional=True)
...
def main():
    ...
    for epoch in range(10):
        ...
        print(f"[CRISPR-Net] Epoch {epoch+1}: Loss={loss:.4f} R2={r2:.4f}")

    mean_pred, std_pred = predict_with_uncertainty(model, test_loader, device, T=50)
    for i in range(5):
        print(f"[CRISPR-Net] Pred: {mean_pred[i]:.4f} ± {std_pred[i]:.4f}, True: {test_labels[i]}")

    torch.save(model.state_dict(), "CRISPRNet_model.pth")
    print("✅ CRISPR-Net model saved.")
